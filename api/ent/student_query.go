...
func (_q *StudentQuery) QueryGuardians() *GuardianQuery {
	query := (&GuardianClient{config: _q.config}).Query()
...
func (_q *StudentQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *StudentQuery) FirstIDX(ctx context.Context) string {
...
func (_q *StudentQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *StudentQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *StudentQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *StudentQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *StudentQuery) Clone() *StudentQuery {
	...
	return &StudentQuery{
		...
		withGuardians: _q.withGuardians.Clone(),
...
func (_q *StudentQuery) WithGuardians(opts ...func(*GuardianQuery)) *StudentQuery {
	query := (&GuardianClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGuardians = query
	return _q
}
...
func (_q *StudentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Student, error) {
	...
	if query := _q.withGuardians; query != nil {
		if err := _q.loadGuardians(ctx, query, nodes,
			func(n *Student) { n.Edges.Guardians = []*Guardian{} },
			func(n *Student, e *Guardian) { n.Edges.Guardians = append(n.Edges.Guardians, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *StudentQuery) loadGuardians(ctx context.Context, query *GuardianQuery, nodes []*Student, init func(*Student), assign func(*Student, *Guardian)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[string]*Student)
	nids := make(map[string]map[*Student]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(student.GuardiansTable)
		s.Join(joinT).On(s.C(guardian.FieldID), joinT.C(student.GuardiansPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(student.GuardiansPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(student.GuardiansPrimaryKey[1]))
	...
	neighbors, err := withInterceptors[[]*Guardian](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "guardians" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
