...
import { YStack, XStack, Text, Card, Switch, H2, Paragraph, Circle } from 'tamagui';
...
const mockNotifications: Notification[] = [
  {
    id: '1',
    title: 'Welcome!',
    message: 'Thanks for joining our app. Get started by exploring the features.',
    time: '2 min ago',
    read: false,
    type: 'success',
  },
    ...
    >
      <Card.Header padding="$4">
        <XStack alignItems="flex-start" marginBottom="$2">
          <Text fontSize="$5" marginRight="$3">{getTypeIcon(item.type)}</Text>
          <YStack flex={1}>
            <Text 
              fontSize="$4" 
              fontWeight="600" 
            ...
            <Text fontSize="$2" color="$color11">{item.time}</Text>
          </YStack>
          {!item.read && (
            <Circle size="$1" backgroundColor="$blue9" marginLeft="$2" marginTop="$1" />
          )}
        </XStack>
        <Paragraph 
          fontSize="$3" 
      ...
      <Card size="$4" margin="$4" marginBottom="$0">
        <Card.Header>
          <XStack alignItems="center" justifyContent="space-between">
            <Text fontSize="$4" fontWeight="500">Push Notifications</Text>
            <Switch
              size="$3"
              checked={pushEnabled}
